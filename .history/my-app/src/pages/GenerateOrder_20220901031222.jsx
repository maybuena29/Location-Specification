...
const { Search } = Input;
  ...
  const [soldCounter,setsoldCounter]=useState(1);
    ...
    const onInputChange = (key, index) => (
      ...
      if (Number(e.target.value)===null||NaN){
        newData[index][key]= "0";
      }
      setTotal(newData, index);
      setobtainSelectedOrder(newData);
      console.log(obtainSelectedOrder);
    };
  ...
  const selectionColumns:ColumnProps<TransacTableData>[] = [
    {
      title: 'productSKU',
    ...
    { 
      ...
      render: (text, record, index) => {
        return (
          <Input value={text}  type="number" defaultValue="0"onChange={onInputChange("TransacSoldQty", index)} />
        )
