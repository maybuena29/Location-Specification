...
const AddAPInvoice = () => {
  ...
  const fetchGRNumberList = async () => {
    await Axios.get(`http://localhost:3001/api/apinvoice/get/pending/grlist`, {signal: abortController.signal}).then((response)=>{
  ...
  const fetchCurrentGoods = async (value) => {
    setGrNumber(value);
    await Axios.get(`http://localhost:3001/api/goodsreceipt/get/${value}`, {signal: abortController.signal}).then((response)=>{
  ...
  const fetchCurrentInvoice = async () => {
    await Axios.get(`http://localhost:3001/api/apinvoice/get/${id}`, {signal: abortController.signal}).then((response)=>{
      setSupplierName(response.data.Supplier);
  ...
  const fetchCurrentGoodsItem = async (currentGR) => {
    await Axios.get(`http://localhost:3001/api/goodsreceipt/get/goodsreceipt/item/${currentGR}`, {signal: abortController.signal}).then((response)=>{
  ...
  async function submitButton(event){
    if(grNumber === '' || grNumber === 'GR Number' || grNumber === 'Choose GR Number'){
      ...
      await Axios.post("http://localhost:3001/api/apinvoice/insert",{
  ...
  const handleUpdateGRStatus = async () => {
    try {
      await Axios.put(`http://localhost:3001/api/apinvoice/update/grstatus/${grNumber}`).then((err) => {
