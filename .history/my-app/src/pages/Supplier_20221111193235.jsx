...
const Supplier = () => {
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/api/supplier/get", {signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(supplierComName.trim().length === 0){
        ...
        Axios.post('http://localhost:3001/api/supplier/insert', {
      ...
      //code for update
      Axios.put(`http://localhost:3001/api/supplier/update/${supID}`, {
          SupplierComName: supplierComName,
          SupplierRepName: supplierRepName,
          SupplierContNum: supplierContNum,
          SupplierAddress: supplierAddress,
  ...
  const showDataToModal = (supplierID) => {
    Axios.get(`http://localhost:3001/api/supplier/get/${supplierID}`).then((resp)=>{
  ...
  const removeSupplier = (supplierID) => {
    if(window.confirm('Are you sure that you wanted to delete this supplier?')){
      Axios.delete(`http://localhost:3001/api/supplier/remove/${supplierID}`);
      openNotif('warning');
      setCount(() => count + 1)
    }
  }
