...
const GenerateOrder = () => {
  ...
  const [printerDevice, setPrinterDevice] = useState(undefined)
  ...
  const handleCheckout = () => {
    if(paymentMode === 'cash'){
      if(payment < totalAmount){
        ...
        setReferenceNum(referenceNum? referenceNum:uniqueid(17));
  ...
  const handleInsertPointing = async () =>{
    let point = totalAmount/200;
    if (point !== 0 && custCode !== 0) {
      try{
        await Axios.post(`${mainApi}/api/points/insert/CP`, {
          RefNumber: referenceNum,
          CustCode: custCode,
          Points: point
        }).then((err) => {
          ...
          console.log('Data added to Pointing...' + err);
        }).catch((err) => {
          alert(err.response.data);
        });
      }catch(e){
        console.log("Error in Pointing: " + e);
      }
  ...
  const handlePaymentModeChange = (value) => {
    if(!(value === 'cash')){
      setPayment(numberFormat(totalAmount));
    }else{
      setPayment(payment);
    }
  }
  ...
  const handleSelectPrinter = async (device, setDevice) => {
  ...
  const selectPrinter = async () =>{
    ...
    try{
      ...
      alert('No Device Selected.\nYou must select a device to print.');
    }
  }
        ...
        <Card
          ...
          cover={card.productImage?
                    <div className="flex items-center justify-center flex-auto p-10 pl-12 pr-12 border-b-1" style={{opacity: card.inventoryQuantity === 0? '0.5':'1'}}>
                      <img alt="Image Not Available" className="w-auto h-28" src={card.productImage}/>
                    </div>
                    :
                    <div className="flex items-center justify-center flex-auto p-10 pl-12 pr-12 border-b-1" style={{opacity: card.inventoryQuantity === 0? '0.5':'1'}}>
        ...
        {/* second table */}
          <div className="p-2 pb-5 m-2 bg-white shadow-md md:m-2 md:px-4 rounded-xl md:w-1/3">
                ...
                <div className='flex flex-col w-1/2 pt-8 bg-white rounded-md shadow-lg'>
                  <div>
                    <p className="w-full my-auto text-base font-bold text-center sm:w-34">
                      Total: {numberFormat(totalAmount) || 0}
                    </p>
                  </div>
                  <div>
                    <p className="w-full my-auto text-base font-bold text-center sm:w-34">
                  ...
                  <div>
                    <p className="w-full mx-auto my-auto text-2xl font-bold text-center sm:w-34">
                      Change: {numberFormat(totalChange) || 0}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <Modal title={'Invoice POS'} visible={receiptModalVisible} onOk={handleCloseModal} onCancel={handleCloseModal} width={250} style={{ top: 30 }}
              footer={[
                <Button key='button' className='inline-flex items-center w-auto py-0 text-black rounded' style={{backgroundColor: "#83D2FF", borderRadius: 5}} onClick={()=>{handleSelectPrinter(printerDevice, setPrinterDevice)}}>
