...
const Attributes = () => {
  ...
  const [attributeName, setAttrName] = useState('');
  var totalValue = 0;
  ...
  const search = (data) => {
    return data.filter((item) =>
      item.Attribute_Name.toLowerCase().includes(searchVal) ||
      item.total_value.toString().includes(searchVal) ||
      item.AttrStatus.toLowerCase().includes(searchVal)
    )
  }
  ...
  const showAddModal = () => {
    setBtnType('Add');
    setModal({Title: 'Add Attribute', Visible: true});
  };
  ...
  const showUpdateModal = () => {
    setBtnType('Update');
    setModal({Title: 'Update Attribute', Visible: true});
  };

  //pag load ng page at show ng data sa table
  useEffect(() => {
    
    Axios.get(`${mainApi}/api/attribute/get`,{signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(attributeName.trim().length === 0){
        setMessage('Please enter attribute!');
        setInpStat('error');
      }
        ...
        Axios.post(`${mainApi}/api/attribute/insert`, {
      ...
      //code for update
      Axios.put(`${mainApi}/api/attribute/update/${attrID}`, {
        AttrName: attributeName,
        AttrStatus: status
      }).then(() => {
        console.log('Data updated successfully');
  ...
  const showDataToModal = (attID) => {
    Axios.get(`${mainApi}/api/attribute/get/${attID}`).then((resp)=>{
        setAttrName(resp.data.Attribute_Name);
        setStatus(resp.data.AttrStatus);
        setAttrID(attID);
    });
      setCount((c) => c + 1);
      showUpdateModal();
  ...
  //pag remove ng brand
  const removeAttribute = async (attID) => {
    await Axios.get(`${mainApi}/api/attribute/get/${attID}`).then((resp)=>{
      totalValue = resp.data.total_value;
      console.log(totalValue);
      if(totalValue > 0){ 
        if(window.confirm('Attribute has an existing ' + totalValue + ' value, continue to delete?')){
          const {data} = Axios.delete(`${mainApi}/api/attribute/remove/${attID}`).then((resp)=>{
        ...
        if(window.confirm('Do you want to delete this attribute?')){
          const {data} = Axios.delete(`${mainApi}/api/attribute/remove/${attID}`);
          openNotif('warning');
          setCount((c) => c + 1);
        }
      }
  ...
  //For the column of the table
    ...
    const attrTblGrid: ColumnsType<DataType> = [
      {
      ...
      },
      {
        ...
        sorter: (a, b) => a.AttrStatus.localeCompare(b.AttrStatus),
        render: (text, record) => {
          return <p style={{color: record.AttrStatus === 'active' ? 'green': 'red', textTransform: 'capitalize'}}>{text}</p>
        }
      },
      {
        key: 'Attr_ID',
        ...
        render:(_,record) =>
          <Space align='end'>
            <Button className='text-black py-1 pl-2.5 pr-0 h-9 rounded inline-flex items-center my-auto w-10 sm:w-10 md:w-32 md:p-4' style={{backgroundColor: "#46E4AC", borderRadius: 5}} onClick={()=>{navigate(`/addattribute/${record.Attr_ID}`);}}>
              <img className='object-scale-down w-auto h-5 mr-2 sm:w-auto md:w-5 sm:mr-0 md:mr-2 sm:mx-auto' src={add}/>
              <p className='invisible text-xs font-semibold text-black md:visible lg:flex sm:invisible'>Add Value</p>
            </Button>
    ...
    const aList = attrList.map(({body,...item}) => ({
      ...item,
      key: item.Attr_ID,
      message: body,
    }))
      ...
      <Header title='Attribute'/>
        ...
        <div className='p-2 pb-5 m-2 bg-white shadow-md md:m-10 md:px-10 rounded-xl'>
          ...
          <div className='flex w-full h-12 mt-2'>
            <p className='w-auto px-4 my-auto font-bold sm:w-34 md:w-72'>Available Attributes</p>
                ...
                <Modal title={modal.Title} visible={modal.Visible} onOk={handleOk} onCancel={handleOk}
                  ...
                  <div className='flex flex-col gap-4'>
                    <div>
                      ...
                      <Input className='rounded-sm' value={attributeName || ''} placeholder="Attribute Name"
                        onChange={(e) => {
        ...
        <div className='relative w-full'>
          ...
          <div className='m-2 my-auto overflow-auto bg-white rounded'>
            <Table bordered className='mt-14' columns={attrTblGrid} pagination={{ defaultPageSize: 10, showSizeChanger: true, pageSizeOptions: ['10', '25']}} dataSource={search(aList)}></Table>
          </div>
