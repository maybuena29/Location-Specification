...
const Tax = () => {
  ...
  useEffect(() => {
    Axios.get(`${mainApi}/api/tax/get`, {signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(taxName.trim().length === 0){
        ...
        Axios.post(`${mainApi}/api/tax/insert`, {
          TaxName: taxName,
          TaxValue: taxValue,
          Status: status
        }).then(() => {
      ...
      //code for update
      Axios.put(`${mainApi}/api/tax/update/${tID}`, {
        TaxName: taxName,
        TaxValue: taxValue,
        Status: status
      }).then(() => {
  ...
  const showDataToModal = (taxID) => {
    Axios.get(`${mainApi}/api/tax/get/${taxID}`).then((resp)=>{
  ...
  const removeTax = (taxID) => {
    if(window.confirm('Are you sure that you wanted to delete this discount?')){
      Axios.delete(`${mainApi}/api/tax/remove/${taxID}`);
      openNotif('warning');
      setCount(() => count + 1)
    }
  }
