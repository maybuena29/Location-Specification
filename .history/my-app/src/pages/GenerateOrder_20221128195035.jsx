...
const GenerateOrder = () => {
  ...
  const fetchDiscounts = async () => {
    const { data } = await Axios.get(`${mainApi}/api/discount/get/disc/active`, {signal: abortController.signal}).catch((err)=>{
      if (err.response.status===403){
        navigate('/NotAuthorizedPage')
      }
  ...
  const fetchTaxes = async () => {
    const { data } = await Axios.get(`${mainApi}/api/tax/get/tx/active`, {signal: abortController.signal}).catch((err)=>{
      if (err.response.status===403){
        navigate('/NotAuthorizedPage')
      }
  ...
  const fetchCategoryList = async () => {
    const { data } = await Axios.get(`${mainApi}/api/category/get/categname/active`, {signal: abortController.signal}).catch((err)=>{
      if (err.response.status===403){
        navigate('/NotAuthorizedPage')
      }
