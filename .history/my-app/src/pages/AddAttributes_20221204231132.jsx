...
const AddAttributes = () => {
  ...
  useEffect(() => {
    Axios.get(`${mainApi}/api/attrvalue/display/${attrid}`,{signal: abortController.signal}).then((response)=>{
      setValList(response.data);
    });
    Axios.get(`${mainApi}/api/attrvalue/get/attrname/${attrid}`,{signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(valName.trim().length === 0){
        ...
        Axios.post(`${mainApi}/api/attrvalue/insert`, {
          ValueName: valName,
          ParentID: attrid,
          Status: status
        }).then(() => {
      ...
      //code for update
      Axios.put(`${mainApi}/api/attrvalue/update/${valID}`, {
        ValueName: valName,
        Status: status
      }).then(() => {
        console.log('Data updated successfully');
  ...
  const showDataToModal = (valueID) => {
    Axios.get(`${mainApi}/api/attrvalue/get/${valueID}`).then((resp)=>{
  ...
  const removeValue = (valueID) => {
    if(window.confirm('Are you sure that you wanted to delete this value?')){
      Axios.delete(`${mainApi}/api/attrvalue/remove/${valueID}`).then((resp)=>{
      ...
      <Header title='Attribute'/>
        ...
        <div className='p-2 pb-5 m-2 bg-white shadow-md md:m-10 md:px-10 rounded-xl'>
          ...
          <div className='flex w-full h-12 mt-2'>
            <p className='w-auto px-4 my-auto font-bold capitalize sm:w-34 md:w-72'>Attribute: {attrName}</p>
...
{/* 
          ...
          <div className='flex flex-row justify-end mt-5'>
            <NavLink to={'/products/attributes'}> 
              <Button action='' type='button' size='sm' style={{backgroundColor: '#747C95'}} className='inline-flex items-center w-auto h-10 py-1 pl-2 pr-2 my-1 mr-3 text-black rounded hover:bg-blue-400 md:w-auto md:p-4'>
