...
import { Area, Bar, Line, measureTextWidth, Pie } from '@ant-design/plots';
...
const SupplierAnalytics = () => {
  ...
  const supplierRecord = {
    data: salesType === 'Daily'? dailySupplier : (salesType === 'Weekly'? weeklySupplier : (salesType === 'Monthly'? monthlySupplier : yearlySupplier)),
    isGroup: true,
    xField: salesType === 'Daily'? 'Day' : (salesType === 'Weekly'? 'Week' : (salesType === 'Monthly'? 'Month' : 'Year')),
    yField: 'TotalSupplierOrder',
    ...
    tooltip: {
      formatter: (data) => {
        return { name: data.SupplierName, value: numberFormat(data.TotalSupplierOrder) };
      },
    },
    startOnZero: true,
    isStack: false,
    ...
    intervalPadding: 250,
    label: {
      formatter: (v) => `${numberFormat(v.TotalSupplierOrder)}`,
      position: 'middle',
      layout: [
        {
        ...
        <div className='w-auto col-span-1 mb-5'>
          <div className='m-2 md:my-10 p-2 md:px-10 pb-5 bg-white rounded-xl shadow-md w-full'>
            <div className='flex w-full h-12 mt-2'>
              <p className='my-auto w-auto sm:w-34 md:w-72 px-4 font-bold'>Supplier Overview</p>
            </div>
