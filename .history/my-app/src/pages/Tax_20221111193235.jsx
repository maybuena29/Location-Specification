...
import { Modal, Input, Select, notification, Space, Table, Pagination } from 'antd';
...
const openNotif = (type) => {
  if(type === 'success'){
    ...
    notification[type]({
      message: 'SUCCESS!',
      description: 'Tax Removed Successfully.',
      duration: 4,
      style: {borderRadius: '5px', boxShadow: '0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)'},
    });
  }else if(type === 'error'){
...
const Tax = () => {
  ...
  const search = (data) => {
    return data.filter((item) =>
      item.Tax_Name.toLowerCase().includes(searchVal) ||
      item.Tax_Value.toString().toLowerCase().includes(searchVal) ||
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/api/tax/get", {signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(taxName.trim().length === 0){
        setMessage('Please enter tax name!');
        setInpStat('error');
      }
      else if(taxValue === 0){
        setMessage3('Please enter tax value!');
        setInpStat('error');
      }
        ...
        Axios.post('http://localhost:3001/api/tax/insert', {
          TaxName: taxName,
          TaxValue: taxValue,
          Status: status
        }).then(() => {
      ...
      //code for update
      Axios.put(`http://localhost:3001/api/tax/update/${tID}`, {
        TaxName: taxName,
        TaxValue: taxValue,
        Status: status
      }).then(() => {
  ...
  const showDataToModal = (taxID) => {
    Axios.get(`http://localhost:3001/api/tax/get/${taxID}`).then((resp)=>{
        setTaxName(resp.data.Tax_Name);
        setTaxValue(resp.data.Tax_Value);
  ...
  const removeTax = (taxID) => {
    if(window.confirm('Are you sure that you wanted to delete this discount?')){
      Axios.delete(`http://localhost:3001/api/tax/remove/${taxID}`);
      openNotif('warning');
      setCount(() => count + 1)
    }
  }
  ...
  //For the column of the table
    ...
    const brandTblGrid: ColumnsType<DataType> = [
      ...
      },
      {
        ...
        dataIndex: 'Tax_Value',
        sorter: {
          compare: (a, b) => a.Tax_Value - b.Tax_Value,
        },
        render: (text, record) => {
          return <p style={{textTransform: 'capitalize'}}>{text}</p>
        }
    ...
    const tList = taxList.map(({body,...item}) => ({
      ...item,
      key: item.TaxID,
      message: body,
    }))
      ...
      <Header title='Tax'/>
        ...
        <div className='m-2 md:m-10 p-2 md:px-10 pb-5 bg-white rounded-xl shadow-md'>
          ...
          <div className='flex w-full h-12 mt-2'>
            <p className='my-auto w-auto sm:w-34 md:w-72 px-4 font-bold'>Available Taxes</p>
                ...
                <Modal title={modal.Title} visible={modal.Visible} onOk={handleOk} onCancel={handleOk}
                  ...
                  <div className='flex flex-col gap-4'>
                    <div>
                      ...
                      <Input className='rounded-sm' value={taxName || ''} placeholder="Tax Name"
                        onChange={(e) => {
                    ...
                    <div>
                      ...
                      <Input type='number' className='rounded-sm' value={taxValue || ''} placeholder="Tax Value"
                        onChange={(e) => {
...
{/* 
          ...
          <div className='m-2 my-auto bg-white rounded overflow-auto'>
            <Table bordered className='mt-14' columns={brandTblGrid} pagination={{defaultPageSize: 10, showSizeChanger: true, pageSizeOptions: ['10', '25']}} dataSource={search(tList)}></Table>
          </div>
