...
import { Modal, Space, Table, notification, Input, Image } from 'antd';
...
const Products = () => {
  ...
  const importExceltoDB =(e) =>{
    ...
    Axios.post("http://localhost:3001/api/importfromExl/product",{
      data:data
    })
    .then((response)=>{
  ...
  const productGrid = [
    ...
    },
    {
      ...
      width: '120',
      align: 'Center',
      sorter: (a, b) => a.productSupplier.localeCompare(b.productSupplier),
      render: (text, record) => {
        return <span style={{ textTransform: 'capitalize' }}>{text}</span>
      }
    },
                ...
                <div className='flex flex-col md:flex-row w-full min-w-screen justify-center items-center'>
                  ...
                  <div className="flex  flex-col gap-4 justify-center w-full md:w-1/2 p-0 sm:w-full">
                    ...
                    <div className="flex textboxes w-full">
                      ...
                      <Input className='my-auto' placeholder="No Value" floatlabeltype="Auto" value={product.supplier} />
                    </div>
