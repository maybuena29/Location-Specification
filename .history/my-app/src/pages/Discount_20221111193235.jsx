...
const Discount = () => {
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/api/discount/get", {signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(discName.trim().length === 0){
        ...
        Axios.post('http://localhost:3001/api/discount/insert', {
      ...
      //code for update
      Axios.put(`http://localhost:3001/api/discount/update/${dID}`, {
        DiscountName: discName,
        DiscountValue: discValue,
        Status: status
      }).then(() => {
  ...
  const showDataToModal = (discID) => {
    Axios.get(`http://localhost:3001/api/discount/get/${discID}`).then((resp)=>{
  ...
  const removeDiscount = (discID) => {
    if(window.confirm('Are you sure that you wanted to delete this discount?')){
      Axios.delete(`http://localhost:3001/api/discount/remove/${discID}`);
      openNotif('warning');
      setCount(() => count + 1)
    }
  }
      ...
      <Header title='Discount'/>
        ...
        <div className='m-2 md:m-10 p-2 md:px-10 pb-5 bg-white rounded-xl shadow-md'>
          ...
          <div className='flex w-full h-12 mt-2'>
            <p className='my-auto w-auto sm:w-34 md:w-72 px-4 font-bold'>Available Discounts</p>
