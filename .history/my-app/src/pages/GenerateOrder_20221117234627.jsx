...
const GenerateOrder = () => {
  ...
  const fetchInventoryItems = async () => {
    const { data } = await Axios.get("http://localhost:3001/api/inventory/get/prod/active", {signal: abortController.signal});
  ...
  const fetchDiscounts = async () => {
    const { data } = await Axios.get("http://localhost:3001/api/discount/get/disc/active", {signal: abortController.signal});
    setActiveDiscount(data);
  }

  const fetchTaxes = async () => {
    const { data } = await Axios.get("http://localhost:3001/api/tax/get/tx/active", {signal: abortController.signal});
    setActiveTax(data);
  }

  const fetchCategoryList = async () => {
    const { data } = await Axios.get("http://localhost:3001/api/category/get/categname/active", {signal: abortController.signal});
  ...
  const handleInsertOrder = async () => {
    ...
    try{
      await Axios.post('http://localhost:3001/api/order/insert', {
  ...
  const handleInsertPayment = async () => {
    ...
    try{
      await Axios.post('http://localhost:3001/api/payment/insert', {
        RefNumber: referenceNum,
        CustCode: custCode,
  ...
  const handleInsertCartToDB = async () => {
    ...
    try{
      ...
      {await Array.from(cart).map((item) => {
        ...
        Axios.put(`http://localhost:3001/api/inventory/update-quantity/${item.inventoryID}`,{
            Quantity: item.quantity,
        }).then((err)=>{
