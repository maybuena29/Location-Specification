...
const Products = () => {
  ...
  const ExportToExcel = () => {
    const exportToXLSX = () => {
        const ws = XLSX.utils.json_to_sheet(productListEXL)
        const fileType = "xlsx"
        const wb = XLSX.utils.book_new();
        
        XLSX.utils.book_append_sheet(wb, ws,"Corumed-Masterlist");
        XLSX.writeFile(wb,"Corumed-Masterlist.xlsx")
  ...
  const fetchEXL = async () => {
    const { data } = await Axios.get(`${mainApi}/api/products/export`, {
      signal: abortController.signal
    }).catch((err)=>{
      if (err.response.status===403){
        navigate('/NotAuthorizedPage')
        ...
        <div className='flex w-full h-12 mt-2'>
          ...
          <div className='relative w-full'>
            <div className='absolute right-0 w-34'>
              ...
              <NavLink to={''}>
                <Button type='button' size='sm' onClick={()=>ExportToExcel(productList)} style={{ backgroundColor: '#46E4AC' }} className='inline-flex items-center w-10 h-10 py-1 pl-2 pr-0 my-1 m-2 text-black rounded hover:bg-green-400 sm:w-10 md:w-auto md:p-4'>
                  <img className='object-scale-down w-6 h-6 mx-auto mr-2 md:w-5 sm:mr-0 md:mr-2 sm:mx-auto' src={impExcel} /><MdFileUpload></MdFileUpload>
