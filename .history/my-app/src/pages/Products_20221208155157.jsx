...
const Products = () => {
  ...
  const ExportToExcel = (Masterlists) => {
    const fileType = "xlsx"
    const exportToCSV = () => {
  ...
  const fetch = async () => {
    ...
    setProductList(data)
    const { dataJson } = await Axios.get(`${mainApi}/api/products/get`, {
      signal: abortController.signal
    }).then(response => response.json())
        .then(json => {   
          setProductListExl(json)
        })  
    .catch((err)=>{
      if (err.response.status===403){
        navigate('/NotAuthorizedPage')
        ...
        <div className='flex w-full h-12 mt-2'>
          ...
          <div className='relative w-full'>
            <div className='absolute right-0 w-34'>
              ...
              <NavLink to={''}>
                <Button type='button' size='sm' onClick={()=>ExportToExcel(productListExl)} style={{ backgroundColor: '#46E4AC' }} className='inline-flex items-center w-10 h-10 py-1 pl-2 pr-0 my-1 text-black rounded hover:bg-green-400 sm:w-10 md:w-auto md:p-4'>
                  <img className='object-scale-down w-6 h-6 mx-auto mr-2 md:w-5 sm:mr-0 md:mr-2 sm:mx-auto' src={impExcel} />
