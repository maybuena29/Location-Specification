...
const Inventory = () => {
  ...
  const fetch = async () => {
    const { data } = await Axios.get("http://localhost:3001/api/inventory/get", {signal: abortController.signal})
      ...
      <Header title='Inventory'/>
        ...
        <div className='m-2 md:m-10 p-2 md:px-10 pb-5 bg-white rounded-xl shadow-md'>
          ...
          <div className='m-2 my-auto bg-white rounded overflow-auto'>
            <Table bordered className='mt-14' columns={inventoryGrid} rowKey={'inventoryID'} pagination={{ defaultPageSize: 10, showSizeChanger: true, pageSizeOptions: ['10', '25', '50', '100']}} dataSource={search(inventoryList)}></Table>
          </div>
