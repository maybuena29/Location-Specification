...
const AddAttributes = () => {
  ...
  useEffect(() => {
    Axios.get(`http://localhost:3001/api/attrvalue/display/${attrid}`).then((response)=>{
      setValList(response.data);
    });
    Axios.get(`http://localhost:3001/api/attrvalue/get/attrname/${attrid}`).then((response)=>{
      ...
      <Header title='Attribute'/>
        ...
        <div className='m-2 md:m-10 p-2 md:px-10 pb-5 bg-white rounded-xl shadow-md'>
          ...
          <div className='m-2 my-auto bg-white rounded overflow-auto'>
            <Table bordered className='mt-14' columns={attrValTblGrid} dataSource={vList} onChange={onChange}></Table>
          </div>
