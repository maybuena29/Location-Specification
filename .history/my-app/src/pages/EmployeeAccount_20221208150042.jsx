...
const EmployeeAccount = () => {
  ...
  const fetchEmployee= async () => {
    const { data } = await Axios.get(`${mainApi}/api/employee/get`, {signal: abortController.signal}).catch((err)=>{
  ...
  const fetchRoleList= async () => {
    const { data } = await Axios.get(`${mainApi}/api/employee/getrolelist`, {signal: abortController.signal})
    setRoleList(data)
...
// }
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      ...
      else if(employeeList.userRoleID === "Employee Role"){
      ...
      if(employeeList.userName.trim().length === 0){
      ...
      else if(employeeList.userPassword.trim().length === 0 && OnOff===false){
        message.error('The Employee Password is Empty!');
        alert()
        console.log("zzzz");
      }
        ...
        message.error('Select an Employee Status!');
         ...
         //code for update
      Axios.put(`${mainApi}/api/employee/update/${employeeList.userID}`, {
        userName: employeeList.userName,
        userContact:  employeeList.userContact,
        userAddress : employeeList.userAddress,
        userRoleID: employeeList.userRoleID,
        ...
        console.log(employeeList.userID);
        if (response.data.message==="Username already Exist"){
          openNotificationWithIconExist("error");
        }
          else{
  ...
  //For the column of the table
    ...
    const employeeTblGrid: ColumnsType<DataType> = [
      ...
      },
      {
        ...
        sortDirections: ['descend', 'ascend'],
        render: (text, record) => {
          return <p style={{color: record.userStatus === 'active' ? 'green': 'red', textTransform: 'capitalize'}} className='font-semibold'>{text}</p>
        }
      },
      {
        key: 'userID',
        ...
        render:(_,record) =>
          <Space align='end'>
            {/* For Update */}
            <Button style={{backgroundColor: '#83D2FF'}}  className='text-black py-1 pl-2.5 pr-0 h-9 rounded inline-flex items-center my-auto w-10 sm:w-10 md:w-10 md:p-3 md:pl-3 sm:pr-2.5' onClick={()=> {showDataToModal(record.userID);}}>
          ...
          {/* <div className='relative w-full'>
            ...
            <Modal title={modal.Title} visible={modal.Visible} onOk={handleOk} onCancel={handleOk}
                  ...
                  <p className='ml-5'>User Employee Informations</p>
                    ...
                    <div>
                    <Select className='w-full rounded-sm' placeholder="Add Role" value={employeeList.userRoleID||null} onChange={(selectedrole) => 
                         {setEmployeeList({...employeeList, userRoleID: selectedrole})} } >
                        {Array.from(roleList).map((rolelist, index) => (
                        <Option key={rolelist.userRoleID} value={rolelist.userRoleID} search={rolelist.userRolename}>{rolelist.userRolename}</Option>
                      ))} 
                      </Select>
                    </div>
                    <div>
                    ...
                    <div>
                      <Input className='rounded-sm' value={employeeList.userUsername|| ''} name="userUsername" disabled={btnType === "Update"? true:false} placeholder="Employee Username" onChange={onChangeEmp} />
                    </div>

                    <div>
                      <Input className='rounded-sm' type="password" value={employeeList.userPassword|| ''} name="userPassword" placeholder="Employee Password" disabled={OnOff} onChange={onChangeEmp}  />
                        {btnType==="Update"?<Checkbox checked={OnOff===true? false:true} onChange={onChangeCB}>Reset?</Checkbox>:null}
                    </div>         
