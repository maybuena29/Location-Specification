...
import { Input, Select,notification,DatePicker } from 'antd';
...
const AddInventory = () => {
  ...
  const retrievedDateManu = moment(inventoryDateManu, dateFormat);
  const retrievedDateExp = moment(inventoryDateExp, dateFormat);
...
Axios.get(`http://localhost:3001/api/inventory/get/sku/${getSKU}`).then((response)=>{
      ...
      console.log("response.data.productName : "+response.data.productName)
      setPName(response.data.productName);
      setPDescription(response.data.productDescription);
      setPAttribute(response.data.productAttribute);
      setPAttrValue(response.data.productAttrValue);
    ...
    <div className='md:m-10 mt-20 mx-8'>
      ...
      <div className='m-2 md:m-10 py-10 px-5 md:px-14 md:pt-5 bg-white rounded-xl shadow-md items-center justify-center'>
        ...
        <div className='flex w-full h-12'>
          <p className='text-lg my-auto w-full px-4 font-bold'>{id? "Update Inventory" : "Add Inventory"}</p>
        ...
        <form action='' className=''>
          ...
          <div className='flex flex-col md:flex-row w-full min-w-screen justify-center items-center'>
            ...
            <div className="flex textboxes w-full">
                  ...
                  {Array.from(obtainedSKU).map((SKUList)=>
                              (
                              <Option value={SKUList.productSKU}>{SKUList.productSKU}</Option>
                            ))}
                  </Select>
              </div>
            ...
            <div className="flex  flex-col gap-4 justify-center w-full md:w-1/2 p-0 sm:w-full">
              ...
              <div className="flex textboxes w-full">
                  ...
                  <Input className='my-auto' type="number" value={inventoryQuantity || ''} placeholder="Enter Inventory Quantity"   disabled={productSKU==="Choose/Search SKU"? true:false} onChange={(e)=> setIQuantity(e.target.value)}/>
              </div>

              <div className="flex textboxes w-full">
                  <p className='my-auto font-display w-32'>Date Manufactured:</p>
                  <DatePicker className='my-auto'  format={"YYYY-MM-DD"} value={id? retrievedDateManu : inventoryDateManu || ''} placeholder="Date Manufactured"   disabled={productSKU==="Choose/Search SKU"? true:false } onChange={handleIDateManuChange}/>
              </div>

              <div className="flex textboxes w-full">
                  <p className='my-auto font-display w-32'>Date Expiration:</p>
                  <DatePicker className='my-auto' format={"YYYY-MM-DD"} value={id? retrievedDateExp : inventoryDateExp || ''} placeholder="Date Expiration"   disabled={productSKU==="Choose/Search SKU"? true:false} onChange={handleIDateExpChange}/>
              </div>
              ...
              <div className="flex textboxes w-full">
                  ...
                  <Input type = "checkbox" defaultChecked={updateCBRP? true:null} value={updateCBRP? true:null} disabled />
              </div>
