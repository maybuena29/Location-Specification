...
const Company = () => {
  ...
  const removeFAQ = (faqID) => {
    if(window.confirm('Are you sure that you wanted to delete this FAQ?')){
      Axios.delete(`http://localhost:3001/api/faq/remove/${faqID}`);
    ...
    if(btnType === 'Add'){
      if(QAfaq.question.trim().length === 0){
        ...
        message.error('Select a FAQ Status!');
         ...
         Axios.post('http://localhost:3001/api/faq/insert', {
      ...
      //code for update
       Axios.put(`http://localhost:3001/api/faq/update/${QAfaq.faqID}`, {
        question: QAfaq.question,
        answer:QAfaq.answer,
        status : QAfaq.status
  ...
  const showDataToModal = (faqID) => {
    Axios.get(`http://localhost:3001/api/faq/get/${faqID}`).then((resp)=>{
        setQAfaq(resp.data);
        UpdateFAQModal();
      });
      fetchFAQ();
...
const fetchCompany = async () => {
  const { data } = await Axios.get("http://localhost:3001/api/company/get", {signal: abortController.signal})
  //setCompany(data)
  setCompany({...data, date_established : `${moment(data.date_established)}`})
}
const fetchFAQ = async () => {
  const { data } = await Axios.get("http://localhost:3001/api/faq/get", {signal: abortController.signal})
  setFAQTable(data)
}
...
async function EditUpdateCompany(e){
  if (EDorUPD ==="Edit"){
    setEDorUPD("Update");
      ...
      const request = await Axios.put("http://localhost:3001/api/company/update",{
        ...
        <div className='m-20 my-auto bg-white rounded overflow-auto'>
            ...
            <div className="my-5 flex textboxes w-full">
               <p className='my-auto font-display w-32'>Company Date Established:</p>
                <DatePicker className='my-auto w-full' name="date_established"value={moment(company.date_established)} placeholder="No Value" disabled={EDorUPD === "Edit"? true:false} onChange={onChangeCompany}/>
            </div>
