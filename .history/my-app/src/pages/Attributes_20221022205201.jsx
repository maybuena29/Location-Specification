...
const Attributes = () => {
  ...
  useEffect(() => {
    ...
    Axios.get("http://localhost:3001/api/attribute/get",{signal: abortController.signal}).then((response)=>{
  ...
  const handleSubmit = () => {
    if(btnType === 'Add'){
      if(attributeName.trim().length === 0){
        ...
        Axios.post('http://localhost:3001/api/attribute/insert', {
      ...
      //code for update
      Axios.put(`http://localhost:3001/api/attribute/update/${attrID}`, {
        AttrName: attributeName,
        AttrStatus: status
      }).then(() => {
        console.log('Data updated successfully');
  ...
  const showDataToModal = (attID) => {
    Axios.get(`http://localhost:3001/api/attribute/get/${attID}`).then((resp)=>{
  ...
  const removeAttribute = async (attID) => {
    await Axios.get(`http://localhost:3001/api/attribute/get/${attID}`).then((resp)=>{
      totalValue = resp.data.total_value;
      if(totalValue > 0){ 
        if(window.confirm('Attribute has an existing ' + totalValue + ' value, continue to delete?')){
          const {data} = Axios.delete(`http://localhost:3001/api/attribute/remove/${attID}`);
          openNotif('warning');
          setCount(() => count + 1);
        }
      }else{
        ...
        render:(_,record) =>
          <Space align='end'>
            <Button className='text-black py-1 pl-2.5 pr-0 h-9 rounded inline-flex items-center my-auto w-10 sm:w-10 md:w-32 md:p-4' style={{backgroundColor: "#46E4AC", borderRadius: 5}} onClick={()=>{navigate(`/addattribute/${record.Attr_ID}`);}}>
              ...
              <p className='invisible text-black text-xs font-semibold md:visible lg:flex sm:invisible'>Add Value</p>
            </Button>
      ...
      <Header title='Attribute'/>
        ...
        <div className='m-2 md:m-10 p-2 md:px-10 pb-5 bg-white rounded-xl shadow-md'>
          ...
          <div className='flex w-full h-12 mt-2'>
            <p className='my-auto w-auto sm:w-34 md:w-72 px-4 font-bold'>Available Attributes</p>
