...
const ViewGoodsReceipt = () => {
  ...
  const fetchPONumberList = async () => {
    await Axios.get(`http://localhost:3001/api/purchaseorder/get/pending/polist`, {signal: abortController.signal}).then((response)=>{
  ...
  const fetchCurrentPO = async (value) => {
    setPoNumber(value);
    await Axios.get(`http://localhost:3001/api/purchaseorder/get/${value}`, {signal: abortController.signal}).then((response)=>{
  ...
  const fetchCurrentGR = async () => {
    await Axios.get(`http://localhost:3001/api/goodsreceipt/get/${id}`, {signal: abortController.signal}).then((response)=>{
      setSupplierID(response.data.Supplier);
  ...
  const fetchCurrentPurchaseItem = async (currentPO) => {
    await Axios.get(`http://localhost:3001/api/purchaseorder/get/purchaseitems/${currentPO}`, {signal: abortController.signal}).then((response)=>{
      addToItemCart(response.data);
    });
  }

  const fetchSupplierName = async (currentPO) => {
    await Axios.get(`http://localhost:3001/api/goodsreceipt/get/supplier/name/${currentPO}`, {signal: abortController.signal}).then((response)=>{
      setSupplierName(response.data.Supplier_ComName);
    });
  }

  const fetchCurrentGoodsItem = async () => {
    await Axios.get(`http://localhost:3001/api/goodsreceipt/get/goodsreceipt/item/${id}`, {signal: abortController.signal}).then((response)=>{
      addToItemCart(response.data);
    });
  }
  ...
  async function submitButton(event){
    if(poNumber === '' || poNumber === 'PO Number' || poNumber === 'Choose PO Number'){
      ...
      await Axios.post("http://localhost:3001/api/goodsreceipt/insert",{
        PoNumber: poNumber,
        GrNumber: grNumber,
  ...
  const handleUpdatePO = async () => {
    try {
      await Axios.put(`http://localhost:3001/api/goodsreceipt/update/postatus/${poNumber}`).then((err) => {
  ...
  const handleAddItemCart = async () => {
    try{
      ...
      {await Array.from(itemCart).map((item) => {
        Axios.post('http://localhost:3001/api/goodsreceipt/insert/goodsitem', {
