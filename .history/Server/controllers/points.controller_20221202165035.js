...
function displayData(req, res){
    const sqlSelect = "SELECT `PntCustID`, `CustCode`, (SELECT CustName FROM tblcustomer WHERE CustCode = 'CustomerCode') AS CustName, (SELECT Contact FROM tblcustomer WHERE CustCode = 'CustomerCode') AS Contact, `TotalPurchased`, `TotalPoints`, `LastUpdate` FROM `tblcustomerpoints` Where CustCode >0;";
...
function insertData(req, res){
    ...
    const PoDateCreated = req.body.DateCreated
    const Remarks = req.body.Remarks
    const Status = 'pending'

    const sqlInsert = "INSERT INTO `tblpurchaseorder`(`PONumber`, `Supplier`, `PODelDate`, `PORemarks`, `PODateCreated`, `POStatus`) VALUES (?, ?, ?, ?, ?, ?)";
