    ...
    upload(req, res, async function(err) {
        ...
        const productSKU = skuProdName+skuProdNameInitials+skuPrice+"-"+skuAttrName+skuAttrValue+"-"+skuBrand+"-"+skuCategory;
            if (!productImage){
                ...
                db.query(sqlUpdate, [productName, productDescription, productAttribute, productAttrValue, productPrice, productCategory, productBrand, productSKU, productAvailability, prodStatus, PID], (err, result) => {
                    if(err){
                        console.log(err);
                    }
                    res.send(result);
