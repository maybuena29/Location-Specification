...
function displayItemDailySalesNL(req, res){
    const sqlSelect = "SELECT (SELECT productName FROM `tblproducts` WHERE productID = OI.productID) AS 'ProductName', DATE_FORMAT(O.Date, '%Y') AS 'Year', DATE_FORMAT(O.Date, '%M') AS 'Month', CONCAT(DATE_FORMAT(O.Date, '%m'), '-', DATE_FORMAT(O.Date, '%d')) AS 'Day', SUM(OI.productPrice * OI.quantity) AS 'TotalSales' FROM `tblorderitems` OI INNER JOIN `tblorders` O ON OI.ReferenceNumber = O.ReferenceNumber GROUP BY OI.productID, DAY(O.Date) ORDER BY O.Date ";
...
function displayDailyEarnings(req, res){
    const sqlSelect = "SELECT SUM(((OI.productPrice - (SELECT productPrice FROM tblproducts WHERE productID = OI.productID)) * OI.quantity) - (((OI.productPrice - (SELECT productPrice FROM tblproducts WHERE productID = OI.productID)) * OI.quantity) * (OI.discount/100))) AS `Total`, DATE_FORMAT(O.Date, '%Y') AS `Year`, DATE_FORMAT(O.Date, '%M') AS `Month`, DATE_FORMAT(O.Date, '%U') AS `Week`, CONCAT(DATE_FORMAT(O.Date, '%m'), '-', DATE_FORMAT(O.Date, '%d')) AS `Day`, (SELECT 'Earnings') AS `Name` FROM tblorderitems AS `OI` INNER JOIN tblorders AS `O` ON OI.ReferenceNumber = O.ReferenceNumber GROUP BY DAY(O.Date) UNION ALL SELECT SUM(O.TotalAmount) AS `Total`, DATE_FORMAT(O.Date, '%Y') AS `Year`, DATE_FORMAT(O.Date, '%M') AS `Month`, DATE_FORMAT(O.Date, '%U') AS `Week`, CONCAT(DATE_FORMAT(O.Date, '%m'), '-', DATE_FORMAT(O.Date, '%d')) AS `Day`, (SELECT 'Sales') AS `Name` FROM tblorders AS `O` GROUP BY DAY(O.Date) ORDER BY `Name` ASC, `Day` ASC";
...
function displayDailyOrders(req, res){
    const sqlSelect = "SELECT COUNT(`OrderID`) AS TotalOrders, DATE_FORMAT(`Date`, '%Y') AS 'Year', DATE_FORMAT(`Date`, '%M') AS 'Month', DATE_FORMAT(`Date`, '%U') AS 'Week', CONCAT(DATE_FORMAT(`Date`, '%m'), '-', DATE_FORMAT(`Date`, '%d')) AS 'Day' FROM `tblorders` GROUP BY DAY(`Date`) ORDER BY `Date`";
...
function displaySupplierDaily(req, res){
    const sqlSelect = "SELECT COUNT(`Supplier`) AS TotalSupplierOrder,(Select Supplier_ComName FROM `tblsupplier` WHERE SupplierID = `Supplier`) AS SupplierName,DATE_FORMAT(`PODateCreated`, '%Y') AS 'Year', DATE_FORMAT(`PODateCreated`, '%M') AS 'Month', DATE_FORMAT(`PODateCreated`, '%U') AS 'Week', CONCAT(DATE_FORMAT(PO.PODateCreated, '%m'), '-', DATE_FORMAT(PO.PODateCreated, '%d')) AS 'Day' FROM `tblpurchaseorder` PO GROUP BY DAY(PO.PODateCreated), SupplierName ORDER BY PO.PODateCreated;";
