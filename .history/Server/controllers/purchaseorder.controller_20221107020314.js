...
function displayData(req, res){
    const sqlSelect = "SELECT `purchaseID`, `PONumber`, (SELECT Supplier_ComName FROM tblsupplier WHERE SupplierID = `Supplier`) AS 'Supplier', `PODelDate`, `PORemarks`, `PODateCreated`, `POStatus` FROM `tblpurchaseorder`";
...
function updateItemStatus(req, res){
    ...
    db.query(sqlInsert, [status, poNumber, prodID], (err, result) => {
        console.log("result sa update status:");
        console.log(result);
        res.send(result);
    });
}
