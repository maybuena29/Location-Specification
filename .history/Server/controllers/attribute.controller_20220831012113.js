...
function getAttributeIDfromName(req, res){
    const sqlSelect = "SELECT Attr_ID FROM tblattributes WHERE Attribute_Name = ?";
    const AttrName = req.params.AttrName;
    db.query(sqlSelect, [AttrName], (err, result) => {
        res.send(result[0]);
        console.log(result[0]);
        console.log(AttrName)
    });
}
// function getAttributeValuefromID(req, res){
//     const sqlSelect = "SELECT Attribute_Name FROM tblattributes WHERE AttrStatus = 'active'";
...
module.exports = {
    ...
    getAttrIDfName: getAttributeIDfromName,
