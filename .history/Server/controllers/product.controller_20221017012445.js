    ...
    upload(req, res, async function(err) {
        ...
        if (err instanceof multer.MulterError) {
            ...
            if (!productImage){
                ...
                db.query(showWarning, (err, result) => {
                    if(err){
                        console.log(err);
                    }
                    res.send(result);
                    console.log(sqlUpdate);
                    console.log([productImage, productName, productDescription, productAttribute, productAttrValue, productPrice, productCategory, productBrand,productSupplier, productAvailability,  prodStatus, PID])
                });
            }
