...
function PointHistory(req,res){
    const sqlInsert ="SELECT OrderID, ReferenceNumber, (SELECT CustName FROM tblcustomer WHERE CustCode = O.CustomerCode) AS CustName, TotalProducts, TotalAmount, TotalTax, TotalDiscount, Date, TIME_FORMAT(Time, '%h:%i %p') as 'Time', TRUNCATE((O.TotalAmount/200), 0) AS TotalPoints, Status FROM tblorders O WHERE O.CustomerCode > 0 AND O.Status = 'paid'"
