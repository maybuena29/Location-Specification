            ...
            async function getBrandName(){
                async function query(){
                   ...
                   const result = await query()
                    return result.replace(/\s+/g, '').substring(0,5).toUpperCase();
             }
             ...
             const productSKU = skuBrand+"-"+skuCategory+"-"+skuAIID;
...
async function updateData(req, res){
    ...
    upload(req, res, async function(err) {
        ...
        if (err instanceof multer.MulterError) {
            ...
            if (!req.file){
                const sqlUpdate = "UPDATE tblproducts SET productName = ?, productDescription = ?, productAttribute = ?, productAttrValue = ?, productPrice = ?,productCategory = ?, productBrand = ?, productSupplier = ?, productAvailability = ?, productSKU = ?, productStatus = ? WHERE productID = ?";

                    db.query(sqlUpdate, [productName, productDescription, productAttribute, productAttrValue, productPrice, productCategory, productBrand,productSupplier, productAvailability, productSKU, prodStatus, PID], (err, result) => {
                        if(err){
                            console.log(err);
                        }
                        res.send(result);
                ...
                const sqlUpdate = "UPDATE tblproducts SET productImage = ?productName = ?, productDescription = ?, productAttribute = ?, productAttrValue = ?, productPrice = ?,productCategory = ?, productBrand = ?, productSupplier = ?, productAvailability = ?, productSKU = ?, productStatus = ? WHERE productID = ?";

                db.query(sqlUpdate, [productImage, productName, productDescription, productAttribute, productAttrValue, productPrice, productCategory, productBrand,productSupplier, productAvailability, productSKU, prodStatus, PID], (err, result) => {
                    if(err){
                        console.log(err);
                    }
                    res.send(result);
