...
function getOrderItems(req, res){
    ...
    "SELECT `itemOrderID`, `inventoryID`, (SELECT `productName` FROM `tblproducts` WHERE `productID` = o.productID) as productName, `productPrice`, SUM(`quantity`) AS quantity, `discount`, `tax` FROM tblorderitems o WHERE ReferenceNumber = ? GROUP BY productID, productPrice";
