...
function ExportData(req, res){
    const sqlSelect="SELECT `productName`, `productDescription`, (SELECT Attribute_Name FROM tblattributes WHERE Attr_ID = `productAttribute`) AS `productAttribute`, (SELECT Value_Name FROM tblattributesvalue WHERE Value_ID = `productAttrValue`) AS `productAttrValue`, `productPrice`, (SELECT Category_Name FROM tblcategory WHERE CategoryID = `productCategory`) AS `productCategory`, (SELECT Brand_Name FROM tblbrand WHERE BrandID = `productBrand`) AS `productBrand`, `productAvailability`, `productStatus` FROM `tblproducts`";
    db.query(sqlSelect,(err,result) => {
       res.send(result);
    });
}
...
async function insertData(req, res){
    ...
    storage: multerAzure({
        connectionString: 'DefaultEndpointsProtocol=https;AccountName=corumedimages;AccountKey=sup146ays1g0jwLvsof1zsQqpJJd2ETHrwZkkGjnbyIAxTL/dUPrNNoEDyGBx/U+5sK0t8bWExQn+AStHXDPEQ==;EndpointSuffix=core.windows.net', //Connection String for azure storage account, this one is prefered if you specified, fallback to account and key if not.
        account: 'corumedimages', //The name of the Azure storage account
        key: 'sup146ays1g0jwLvsof1zsQqpJJd2ETHrwZkkGjnbyIAxTL/dUPrNNoEDyGBx/U+5sK0t8bWExQn+AStHXDPEQ==', //A key listed under Access keys in the storage account pane
