...
<<<<<<< Updated upstream:.history/Server/controllers/inventory.controller_20221024162237.js
...
>>>>>>> Stashed changes:.history/Server/controllers/inventory.controller_20221107014517.js
    });
}
    ...
    db.query(sqlDelete, [invID],(err,result)=>{
        if(err){
            ...
            logs={
                AuditUserCode:req.session.user.id,
                AuditModule:"Inventory",
                AuditAction:`Deleted an Inventory from ${invID} `
            }
            audit.logger(logs);
            console.log(result);
            res.send(result);
