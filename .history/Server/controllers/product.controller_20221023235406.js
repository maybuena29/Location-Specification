        ...
        if (err instanceof multer.MulterError) {
           ...
           // console.log("product supplier: " + productSupplier);
            if (!productImage){
                ...
                const sqlUpdate = "UPDATE tblproducts SET productName = ?, productDescription = ?, productAttribute = ?, productAttrValue = ?, productPrice = ?, productCategory = ?, productBrand = ?, productAvailability = ?, productStatus = ? WHERE productID = ?";

                db.query(sqlUpdate, [productName, productDescription, productAttribute, productAttrValue, productPrice, productCategory, productBrand, productAvailability, prodStatus, PID], (err, result) => {
                    if(err){
                        console.log(err);
                    }
                    res.send(result);
