...
function insertData(req, res){
    ...
    const sqlInsert = "INSERT INTO `tblcustomerpoints`(`CustCode`, `CurrentPoints`, `LastUpdate`) VALUES (?, ?, (SELECT now())) ON DUPLICATE KEY UPDATE `CurrentPoints` = (`CurrentPoints` + ?), `LastUpdate` = (SELECT now())";
    console.log(point);
    console.log(CustCode);
    db.query(sqlInsert, [CustCode, point,point], (err, result) => {
        if (err){
        console.log(err);
        res.send(err);
        }
...
function PointHistory(req,res){
    const sqlInsert ="SELECT OrderID, ReferenceNumber, (SELECT CustName FROM tblcustomer WHERE CustCode = O.CustomerCode) AS CustName, TotalProducts, TotalAmount, TotalTax, TotalDiscount, Date, TIME_FORMAT(Time, '%h:%i %p') as 'Time', TRUNCATE((O.TotalAmount/200), 0) AS PointsAccumulated, Status FROM tblorders O WHERE O.CustomerCode > 0 AND O.Status = 'paid'"
