...
function AddfromExcel(req, res){
    ...
    function AddInv(){
    data.map((rows)=>{
        const sqlInsert = "INSERT IGNORE INTO tblinventory (productID, inventoryStatus,inventoryQuantity,inventoryDateManu,inventoryDateExp) Values ((SELECT productID FROM tblproducts WHERE productSKU = ?), ?, ?, ?, ?) ON DUPLICATE KEY UPDATE productID =? , inventoryStatus=? ,inventoryQuantity= ?,inventoryDateManu=?,inventoryDateExp=?";
